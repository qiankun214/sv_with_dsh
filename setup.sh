...
#
# 为什么不用 apt：apt-get install 需要 root，而本机 NoNewPrivs=1 封掉了 sudo；
# 且 APT 的版本也不够用（verible 无包、verilator 5.020 < cocotb 要求的 5.036、
# yosys 0.33 读不了本仓 RTL）。apt-get download + dpkg-deb -x 虽不需要 root，
# 但同样受版本限制。详见 docs/setup/toolchain.md。
#
# 用法：
...
setup_venv() {
  if [[ -x "$VENV/bin/python" && $FORCE -eq 0 ]]; then
    ...
    if have uv; then
      ...
      python3 -m venv "$VENV" \
        || die "创建 venv 失败：先装 uv（curl -LsSf https://astral.sh/uv/install.sh | sh）"
...
setup_eda() {
  ...
  if [[ "$(printf '%s\n5.036\n' "$vv" | sort -V | head -1)" != "5.036" ]]; then
    warn "cocotb 2.x 要求 Verilator ≥ 5.036，当前 $vv：gate 05 的 verilator 后端会失败（icarus 可兜底）"
  fi
