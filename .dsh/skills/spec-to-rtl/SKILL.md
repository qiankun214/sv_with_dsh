...
**每一份阶段文档（REQ / ARCH / DES / REGMAP / IFACE / VP / CHK）动笔前，先用 `grill-me` 与用户做一轮质询式讨论。**

- 入口是 `grill-me`，实际执行 `grilling` 技能：**每轮一次 `ask_user_question`**，把该轮决策点做成卡片
  （稳定 `id` + 短 `header`，推荐项放第一并标「（推荐）」+ 一句取舍），用户也可以直接写自己的答案；
- **事实自己查**：模板、上游产物、`tools/sv.py` 规则、`AGENTS.md`、`standards/` 能回答的，不要拿去问用户；
  已定决策涉及的文档要重读一遍，发现矛盾先报告、不要在过时结论上继续；
- 讨论到「没有未决问题、且用户确认理解一致」为止；结论写进产物正文/变更历史，**不要**把讨论过程抄进产物；
- 讨论中形成的规范级结论（新的结构/写作约定）必须回流到对应阶段技能或模板，不能只留在当轮对话里。

## 文档写作通则（所有阶段）

- **只写本阶段新增的结论**：上游已经写过的内容（功能点、端口表、接口协议、时序约束等）用**引用标记**
  （如「见 `REQ-001` 硬件接口」），**不要重述、不要复制表格**——重述一定会随上游改动而漂移。
- **通用约定写进技能/模板**：布局、命名、格式、判据、流程等不随产物变化的内容放在 skill 或 template 里，产物正文不重复一遍。
- **不写废话**：没有新信息的小节宁可不写；正文每一条都要能回答「这一步新增了什么决策」，且能被下游直接使用或核对。

各阶段的**写作硬要求**写在对应技能里；功能点（REQ）这一套最完整，见 `spec-to-rtl-req` 的
「本仓固化的写作约定」（章节顺序、软硬件接口结论式写法、硬件接口三段式、接口协议只写接口、时序图只画端口、F/AC 分工）。
...
- ① 功能点写完（IPO 是否完整、`F*` 是否可验证、non-goals 是否清楚？**验收标准此时不写，留给 ⑤ 扇出**）
- ② 系统方案写完（模块划分与预算是否认可？）
- ③ 详细设计写完（端口表与 FSM 是否可作为 RTL 契约？）
- ⑤ 自测计划与回归结果（验收标准是否已从 `REQ-*` 的 `F*`/接口不变式/时序场景扇出？覆盖率是否接受？）
